...
public abstract class MsgBodyCreator {
  ...
  /**
   * 根据实际消息体返回对应的神策 $PlanMsgSendDone 事件埋点属性 Map；如果需要对消息体进行神策 $PlanMsgSendDone 事件埋点增加消息属性，请在子类中实现此方法。
   *
   * 事件属性规范：
   ...
   * 此接口返回的事件属性影响：只影响 $PlanMsgSendDone 事件筛选，SF 核心逻辑不依赖于这里的属性返回值，但若不返回则 $PlanMsgSendDone 事件中就没有，某些筛选条件是无效的.
   *    比如：已经配置好的计划中使用了 $sf_link_url ，如果没有返回这个属性的埋点值，则无法匹配出对应的结果数据，会对统计结果造成影响
   *
   * @param msgBody    实际发送消息体，例如：{templateId: "abc", templateContent: "您好，欢迎采购我们的产品。"}
   * @return 神策 $PlanMsgSendDone 事件埋点需要的属性 Map
